...
class Transformer(nn.Module):
    ...
    def forward_prompt(self, x, prompt):
        l_length = len(self.resblocks)
        p_length = len(prompt) // l_length
        
        prompt = prompt.reshape(p_length, l_length, -1)
        prompt = prompt.permute(1, 0, 2)
        prompt = prompt.unsqueeze(2).repeat(1, 1, x.shape[1], 1)
        # prompt = prompt.unsqueeze(1).repeat(1, x.shape[1], 1)
        ...
        for ind, block in enumerate(self.resblocks):
            ...
            if ind == 0:
                ...
                spatial = x[1+p_length:, :, :]
            x = torch.cat([cls, prompt[ind, :], spatial], 0)
            # x = torch.cat([cls, prompt, spatial], 0)
            x = block(x)
        return x
...
class VisionTransformer(nn.Module):
    ...
    def forward_prompt(self, x: torch.Tensor, prompt):
        ...
        if prompt.ndim == 4:
            ...
            prompt = prompt.repeat(len(x), 1, 1)
    ...
    def forward_prompt_deep(self, x: torch.Tensor, prompt):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat([self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x], dim=1)  # shape = [*, grid ** 2 + 1, width]
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND
        x = self.transformer.forward_prompt(x, prompt)
        x = x.permute(1, 0, 2)  # LND -> NLD
...
class CLIP(nn.Module):
    ...
    def encode_image_prompt(self, image, prompt):
        return self.visual.forward_prompt(image.type(self.dtype), prompt)
    
    def encode_image_prompt_deep(self, image, prompt):
        return self.visual.forward_prompt_deep(image.type(self.dtype), prompt)
